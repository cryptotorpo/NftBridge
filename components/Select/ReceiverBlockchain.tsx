...
import Select, { components, SingleValueProps, OptionProps as DefaultOptionProps } from 'react-select';
...
const getCustomStyles = (theme: string) => ({
  ...
  option: (base: any, { isFocused, isSelected }: any) => ({
    ...base,
    backgroundColor: isSelected ? (theme === 'dark' ? '#272727' : '#ddd') : isFocused ? (theme === 'dark' ? '#333333' : '#eee') : 'transparent',
    color: theme === 'dark' ? '#fff' : '#333',
    cursor: 'pointer',
    '&:active': {
...
const customSingleValue = (props: CustomSingleValueProps) => (
  <div className="flex items-center absolute left-[10px]">
    <Image src={props.data.logo} alt={props.data.label} width={30} height={30} style={{ marginRight: 10, borderRadius: 50 }} />
    {props.data.label}
  </div>
);
...
export const ReceiverBlockchain: React.FC<Props> = ({ onChange, value }) => {
...
export default ReceiverBlockchain;
